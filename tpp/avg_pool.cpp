        ...
        for(unsigned w=0; w<W_; w++)
          for(unsigned i=0; i<KernelSizeH; i++)
            for(unsigned j=0; j<KernelSizeW; j++) {
              ...
              if((i==0) && (j==0)) {
                surface[h][w] = InFmap[batch_idx][c][h*KernelStrideH + i][w*KernelStrideW + j] / (KernelSizeH*KernelSizeW);
              }
              else {
                surface[h][w] = std::min(surface[h][w], InFmap[batch_idx][c][h*KernelStrideH + i][w*KernelStrideW + j]) / (KernelSizeH*KernelSizeW);
              }
            }
